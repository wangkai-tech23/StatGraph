...
def main():
    ...
    for tag in range(1, 5):
        ...
        for i in range(1, range0dic[tag] + 1):  # 表示有   rangedic[tag]张图片
            count += 1
            if count < batch_size:
                m = i + startdic[tag]
                tpath = os.path.join(r'../data9_9_3-each27/test/0/' + str(tag) + '_' + str(
                    i) + '.png')
    ...
    for tag in range(1, 5):
        ...
        for i in range(rangedic[tag]):
            ...
            if count < batch_size:
                m = i + startdic[tag]
                tpath = os.path.join(r'../../data9_9_3-each27/test/' + str(tag) + '/' + str(
                    m) + '.png')
                fopen = Image.open(tpath)
                transform = transforms.Compose([transforms.ToTensor(),
                                                transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])])
